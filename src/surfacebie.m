    ...
    methods
        ...
        function L = surfacebie(n, xfun, yfun, zfun, opts)
            ...
            if ( L.zk == 0 )
                ...
                L.Skern = kernel('helmholtz', 's', sqrt(L.zk));
                L.Dkern = kernel('helmholtz', 'd', sqrt(L.zk));
            end
        ...
        function u = solve(L, f)
            ...
            opts = [];
            opts.accel = false; % Change to true if you have FMM
...
function chnkr = squarechunker(n, nref, rect)
    ...
    for k = 1:nch
        ...
        vx = (verts(k+1,1)-verts(k,1)) / len;
        vy = (verts(k+1,2)-verts(k,2)) / len;
